...
pugi::xml_node XMLBuilder::loadXMLFile()
{
  ...
  pugi::xml_parse_result result = m_documentPointer->load_file(m_fileName.c_str());
																	 ...
																	 T (XMLBuilder::*f)(const pugi::xml_text&)const) const
{
  ...
  for (pugi::xml_node node=parrentNode.first_child();
    ...
    list.push_back((*this.*f)(node.text()));
  }
...
std::vector<float> XMLBuilder::getFloatList(const pugi::xml_node& i_node,
					      ...
					      const int& i_numberOfEntries) const
{
  return getList<float>(i_node,i_xmlPath,i_numberOfEntries,&XMLBuilder::as_float);
}
...
std::vector<std::string> XMLBuilder::getStringList(const pugi::xml_node& i_node,
						     ...
						     const int& i_numberOfEntries) const
{
  return getList<std::string>(i_node,i_xmlPath,i_numberOfEntries,&XMLBuilder::as_string);
}
...
std::vector<bool> XMLBuilder::getBoolList(const pugi::xml_node& i_node,
						   ...
						   const int& i_numberOfEntries) const
{
  return getList<bool>(i_node,i_xmlPath,i_numberOfEntries,&XMLBuilder::as_bool);
}
