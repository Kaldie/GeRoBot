...
bool BaseJointItem::construct(){
  return addBaseMotor() && addEndStops() && createChilderen(BaseJointItem::propertyList);
}
...
bool BaseJointItem::addEndStops() {
  if (!m_jointPointer) {
    return false;
  }

  LOG_DEBUG("Creating EndStop items");
  BaseJoint::EndStopVector vector(m_jointPointer->getEndStops());
  int position(0);
  EndStopItem* endStopItem;
  for (const auto& endStop : vector) {
    endStopItem = new EndStopItem(this, endStop);
    endStopItem->construct();
    insertChild(position, endStopItem);
    ++position;
  }
  return true;
}
