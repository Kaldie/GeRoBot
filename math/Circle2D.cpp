...
{
	if(!verifyPointOnCircle(m_firstPoint) || !verifyPointOnCircle(m_secondPoint))
...
Circle2D::Circle2D(const Point2D& i_firstPoint,
	...
	m_secondPoint(i_secondPoint)
{
	m_centrePoint= this->getCentrePoint(i_firstPoint,i_secondPoint,i_radius,i_isClockwise);
}
...
bool Circle2D::verifyPointOnCircle(const Point2D& i_point) const {
